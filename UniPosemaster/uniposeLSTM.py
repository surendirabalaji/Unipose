...
class Trainer(object):
    def __init__(self, args):
        ...
        self.train_loader, self.val_loader,test_loader = getDataloader(self.dataset, self.train_dir, self.val_dir, self.sigma, self.stride, self.frame_memory,self.batch_size)

        model = unipose(num_classes=self.numClasses,backbone='resnet',output_stride=16,sync_bn=True,freeze_bn=False, stride=self.stride,dataset=self.dataset)
...
if args.dataset == 'Penn_Action':
    args.train_dir  = 'Penn_Action\Penn_Action'
    args.val_dir    = 'Penn_Action\Penn_Action'
